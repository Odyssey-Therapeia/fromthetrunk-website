...
- **Next concrete action**: P2-00/01/02/04/10/11 DONE (P2-04 7307d4e — repaired: calculateOrderTotals now the single charge source, both routes wired, flag-OFF mutation-proven byte-identical; P2-04a client-server-totals deferred). P2-05, P2-06, P2-07 DONE (P2-07 355715d — analytics-sink pipeline, fire-and-forget, events table). Next: **P2-09 structured logger** — lib/log.ts (level/namespace/JSON-in-prod) replacing the ~16+ ad-hoc console.* in lib/+api/ (incl. the new emit.ts [analytics:emit] logs); onError (P1-09 lib/http/on-uncaught-error.ts) routes through it; logging-only, no behavior change. Then P2-08 Payload excision (ISOLATED, mutates/removes deps — LAST in P2). Then #G-P2 evidence (batched), then P3∥P4. **Loop discipline: ONE code packet per wave; workflow scripts PLAIN JS; EXCLUDE .env.production.example + untracked labs/ from commits.**
