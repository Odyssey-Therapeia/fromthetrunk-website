...
- **Next concrete action**: P2-00/01/02/10/11 DONE. **P2-04 was REJECTED** (calculateOrderTotals is dead code — real order-charge math is inline+DUPLICATED in api/hono/routes/payments.ts + orders.ts, unflagged; flag-gating the dead fn changed nothing). **Repair in flight (wy6333j0l)**: consolidate the inline math into calculateOrderTotals, route BOTH routes through it so FTT_FEATURE_GST_INCLUSIVE controls the REAL charged amount; flag-OFF byte-identical to today; client shows server totals (flag not NEXT_PUBLIC). On ACCEPT commit P2-04. Then P2-05 inventory → P2-06 rate-limiter → P2-07 analytics-sink → P2-09 logger → P2-08 Payload (ISOLATED, mutates deps). **Loop discipline: ONE code packet per wave; workflow scripts PLAIN JS (no TS/nested-backticks/angle-brackets).**
