...
- **Next concrete action**: **P2-00/01/02/10/11 DONE** (P2-02 541a64a; P2-02a full schema-DRIVEN SchemaForm DEFERRED = prereq for P3-05/P4-02). Next: **P2-04 GST-inclusive pricing** — behind a `FTT_FEATURE_GST_INCLUSIVE` env flag (default OFF = current behavior); when on, treat pricePaise as GST-inclusive + remove the checkout +12% add-on, consistent across PDP/cart/checkout(calculateOrderTotals)/JSON-LD/emails; L2 math tests both flag states. Then P2-05 inventory → P2-06 rate-limiter → P2-07 analytics-sink → P2-09 logger → P2-08 Payload (ISOLATED, mutates deps). **Loop discipline: code packets run ONE-per-wave** (concurrent workers share the tsc/test surface → spurious failures; only docs/spikes batch). Workflow scripts must be PLAIN JS (no TS annotations / nested backticks / angle brackets).
