...
- **Active phase**: P1 — `plans/P1-stabilize.md`. P1-01..P1-14, P1-16..P1-20 done; only **#G-P1 (user gate)** + P1-15 (ops) remain. Merged tree verified CI-green: clean-worktree `tsc` clean + **231 tests pass**.
- **Branch reality**: `development` (merge 4d1ecf6) = `main` + ALL P1 + emergency guest-checkout fix; **48 ahead / 0 behind origin/main** (conflict-free). `sprint-abe` synced to match. Production = `main` still LACKS guest-checkout until #36 merges.
- **PRs**: **#36 `development → main` OPEN — this is the #G-P1 prod-deploy gate. DO NOT auto-merge; user merges after preview e2e + smoke.** #35 (sprint-abe→development) MERGED (6f371a9). #28 closed.
- **P1-20 main-leg done (this session)**: merged origin/main's 9 commits into development (4d1ecf6). Money path → ours (payment-link/guest/atomic supersedes main's auth-gated Orders flow; main migration 0003 kept). admin/orders/[id] → main's server component + P1-05 status editor ported as client island (`order-status-editor.tsx`). admin list → main's. analytics → storefront only. drizzle journal → both migrations.
- **Next concrete action**: **#G-P1** — review #36 diff, preview-deploy guest-checkout e2e (browse→checkout→payment link), confirm Razorpay env in prod, then USER merges #36 → live smoke. Then P1-15 (unpublish test product). Post-deploy follow-ups: Xeno relocation+redaction (`lib/xeno/dispatcher.ts` channel ID `C0B4S6V22LE`→env + strip "Meena"/"Rekha"; `context-capsule.ts` name+financial fact; `scripts/onboarding/...` `gkarthik@outlook.com`); admin-orders-LIST rewrite as its own packet (in `git stash` — "P1-20 deferred"); full drizzle-meta reconciliation (0002_agent_panel out-of-band).
