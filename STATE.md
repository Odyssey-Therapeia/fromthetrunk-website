...
- **Next concrete action**: P2-00/01/02/04/10/11 DONE (P2-04 7307d4e — repaired: calculateOrderTotals now the single charge source, both routes wired, flag-OFF mutation-proven byte-identical; P2-04a client-server-totals deferred). P2-05, P2-06 DONE (P2-06 02ce7fb — rate-limiter port, in-memory default + env-gated Upstash). Next: **P2-07 server-event pipeline** — lib/ports/analytics-sink.ts + adapters (internal-events→new events table [IF-NOT-EXISTS migration idx5], ga4-measurement-protocol, meta-capi w/ event_id dedup); emit order_created (create-order), payment_completed (completePaidOrder P1-04 winner branch — exactly-once), reservation_expired (cron); FIRE-AND-FORGET, never blocks money path; GA4/Meta creds BATCHED (default internal-events; externals env-gated). Then P2-09 logger → P2-08 Payload (ISOLATED, mutates deps). **Loop discipline: ONE code packet per wave; workflow scripts PLAIN JS; EXCLUDE .env.production.example + untracked labs/ from commits.**
