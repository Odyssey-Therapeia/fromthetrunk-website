...
- **Next concrete action**: **P2-00/10/11 + P2-01 keystone DONE** (b9e5154, mutation-proven). Next: **P2-02 schema-form renderer** (components/admin/schema-form over shadcn per docs/design-system.md; replace product-stepper details step, kill its 15 anys) — wave-3 launching; L3 e2e deferred to #G-P2 demo. Then P2-04 GST → P2-05 inventory → P2-06 rate-limiter → P2-07 analytics-sink → P2-09 logger → P2-08 Payload (ISOLATED, mutates deps). **Loop discipline: code packets run ONE-per-wave** (concurrent workers share the tsc/test surface → spurious failures; serialize for unattended reliability; only docs/spikes batch).
