...
- [x] DONE except #G-P1 (2026-06-13):
  ...
  - [x] **MAIN LEG — `development → main`** (DONE 2026-06-13, merge 4d1ecf6; clean-worktree tsc clean + 231 tests; development 48 ahead / 0 behind main; PR #36 open as the #G-P1 gate; sprint-abe synced): merged `origin/main` (9 commits: #34 vercel-analytics, #32 admin-orders-crash + server-render, #30 razorpay-order-db-compat, #29 razorpay-hardening). **Conflict map (25 hunks / 7 files)** + decisions applied (admin/orders/[id] island file = `order-status-editor.tsx`):
